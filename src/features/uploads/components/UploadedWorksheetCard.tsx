...
export function UploadedWorksheetCard({ upload, categoryName }: UploadedWorksheetCardProps) {
  return (
    <Link
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-1.5 min-w-0">
          <CustomIcon />
          <h3 className="truncate text-base font-semibold text-gray-900 leading-snug">{upload.title}</h3>
        </div>
        <span className={cn('shrink-0 rounded-full px-2 py-0.5 text-xs font-medium', levelColors[upload.level])}>
          {upload.level}
        </span>
      </div>

      <div className="mt-2 flex flex-wrap gap-2 text-xs text-gray-500">
        <span className="rounded bg-orange-100 px-2 py-0.5 text-orange-800 font-medium">My Upload</span>
