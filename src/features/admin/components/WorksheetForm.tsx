  ...
  useEffect(() => {
    if (defaultValues) {
      Object.entries(defaultValues).forEach(([key, val]) => {
        setValue(
          key as keyof WorksheetFormInput,
          val as WorksheetFormInput[keyof WorksheetFormInput],
        );
      });
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        ...
        />
        {errors.author && (
          <p className="text-xs text-red-600 mt-0.5">{errors.author.message}</p>
        )}
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          className={cn(
            "w-full rounded-md border px-3 py-2 text-sm font-mono resize-y",
            errors.answerContent ? "border-red-500" : "border-gray-300",
          )}
        />
